...
public class NetworkedObjects : MonoBehaviour
{
	...
	void Start() {
		...
		float xRange = Random.Range( -world.bounds.extents.x, world.bounds.extents.x );
		float yRange = Random.Range( -world.bounds.extents.x, world.bounds.extents.x );
	...
	public void AddPlayer( PhotonView player ) {
		...
		// only the "server" has authority over which color the player should be
